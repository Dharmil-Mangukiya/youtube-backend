...
const registerUser = asyncHandler( async (req,res)=>{
    ...
    //check if is alredy exists: username and email 
    ...
    if(!avatarLocalPath){
        throw new ApiError(400,"Avatae file is required")
    }
...
const loginUser = asyncHandler(async (req,res)=>{
    //get req body from user
    //which username or email validation 
    //find the user
    //password check
    ...
    const loddedInUser = User.findById(user._id).select("-password -refreshToken")
...
const updateAccountDetails = asyncHandler(async(req,res)=>{
    ...
    return res
    .status(200)
    .json(new ApiResponse(200,user,"New Account Detail successfully"))
